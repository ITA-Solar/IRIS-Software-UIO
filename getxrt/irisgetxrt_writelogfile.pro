;+
; NAME:
;       IRISgetXRT_writelogfile
;
; PURPOSE:
;       IRISgetXRT_writelogfile writes a log file for the process IRIS_getXRTdata
;
; CATEGORY:
...
;
; CALLING SEQUENCE:
;       IRISgetXRT_writelogfile, hinodelog, obs2fov, filename [, traceback=traceback]
;
; INPUTS:
...
;       filename: filename of logfile, including full path, but without an extension
;
; OPTIONAL KEYWORD PARAMETERS:
;       tracebacke: the traceback of an error to be saved
;
; OUTPUTS:
;
; CALLS:
...
;
; $Id: irisgetxrt_writelogfile.pro,v 1.1 2019/02/15 13:35:45 mawiesma Exp $  ;



pro IRISgetXRT_writelogfile, hinodelog, obs2fov, filename, traceback=traceback

  if keyword_set(traceback) then begin
    file=filename+'.errorlog'
    OpenW, unit, file, /get_lun
    if N_ELEMENTS(hinodelog) gt 0 then begin
      printf, unit, 'HINODE XRT proc version: ' + strcompress(string(hinodelog.procversion), /remove_all)
  ...
  if N_ELEMENTS(hinodelog) gt 0 then begin
    ...
    OpenW, unit, file, /get_lun
    log = 'Log of XRT2IRIS, proc version: ' + strcompress(string(hinodelog.procversion), /remove_all)
    printf, unit, log
    log = 'l1to2xrt version: ' + hinodelog.version
    printf, unit, log
    ...
    if percent ge 99.95 then format='(f4.0)' else format='(f4.1)'
    log = 'reading            : ' + IRIS_humanreadabletime(hinodelog.tread, totaltimetextlength=totaltimetextlength) + $
      '   (' + string(percent, format=format) + '%)'
    printf, unit, log
