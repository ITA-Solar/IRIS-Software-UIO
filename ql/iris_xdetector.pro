...
; PURPOSE:
;
;       IRIS_XDETECTOR is used to display 2-D (or higher) data. It is a
...
;
; CALLING SEQUENCE:
;       iris_xdetector, data, lindx, group_leader = groupleader, ncolors=ncolors
;
; INPUTS:
...
;
; PROCEDURE:
;       IRIS_XDETECTOR defines the widgets and displays data. It has several
...
pro iris_xdetector_ps,event
  thisfile=dialog_pickfile(/write,file='iris_xdetector.ps')
  ...
  iris_xdetector_draw, pseudoevent
  device,/close_file
  set_plot,thisdevice
end

; save as jpeg file
pro iris_xdetector_jpeg,event
  thisfile=dialog_pickfile(/write,file='iris_xdetector.jpg')
  if thisfile eq '' then return
  widget_control,event.top,get_uvalue=info
  wset,(*info).wid
  snapshot=tvrd()
...
function iris_xdetector_dwoption, event
  widget_control, event.top, get_uvalue = info
...
function iris_xdetector_wloption, event
  widget_control, event.top,get_uvalue=info
  case event.value of
  0: iris_xdetector_wpix, event
  1: iris_xdetector_wangstr, event
  endcase
  return,0
end

; slit scale selection buttons
function iris_xdetector_sloption, event
  widget_control, event.top,get_uvalue=info
  case event.value of
  0: iris_xdetector_spix, event
  1: iris_xdetector_sarcsec, event
  endcase
  return,0
end

; slider to select exposure number
pro iris_xdetector_expslider, event
  widget_control, event.top,get_uvalue=info
...
; display new raster position
  ...
  widget_control, event.top, set_uvalue=info
  iris_xdetector_resize, pseudoevent
end

; slider to select exposure within a raster pos (if multiple)
pro iris_xdetector_expprp_slider, event
  widget_control, event.top,get_uvalue=info
...
function iris_xdetector_drawsizeoption, event
  widget_control, event.top, get_uvalue = info
...
; pr. rast. pos)
pro iris_xdetector_rast_slider, event
  widget_control, event.top,get_uvalue=info
...
pro iris_xdetector_pickline, event
  widget_control, event.top ,get_uvalue = info
...
function iris_xdetector_pickline_pick, event
  widget_control, event.top, get_uvalue = info
  defdir = ''
...
pro iris_xdetector_pickline_destroy, event
  widget_control, event.top, get_uvalue = info
   iris_xdetector_anim, event
...
pro iris_xdetector_control_anim,event
  widget_control, event.top, get_uvalue = info
  if (*info).nwin lt 2 then begin
    (*info).line=(*info).lindx[0]
    iris_xdetector_anim, event
  endif else begin
    iris_xdetector_pickline, event
  endelse
end

; create animation widget and launch animation
pro iris_xdetector_anim, event
  widget_control, event.top, get_uvalue = info
  magnification=0.95
  minsize=400.0
  maxsize=800.0
...
;;   repeat begin
;;     ct=ct+1
;;     assoc_file = IRISxfiles_appReadme()+'/iris_xdetector_ximovie_'+strtrim(string(ct),2)+'.tmp'
...
pro iris_xdetector_wangstr, event
  widget_control, event.top, get_uvalue = info
   ; change titles in aux object
...
pro iris_xdetector_sarcsec, event
  widget_control, event.top, get_uvalue = info
; change titles in aux object
...
; select color table
pro iris_xdetector_colors, event
  widget_control, event.top, get_uvalue=info
  thisevent = tag_names(event, /structure_name)
  case thisevent of
  'WIDGET_BUTTON': begin
      xcolors, ncolors = (*info).ncolors, bottom = (*info).bottom, $
        title = 'iris_xdetector colors (' + strtrim((*info).wid, 2) + ')', $
...
; protect colors
pro iris_xdetector_protect_colors,event
  widget_control, event.top, get_uvalue = info
  tvlct, (*info).r, (*info).g, (*info).b, (*info).bottom
end

; resize main window
pro iris_xdetector_resize, event
  widget_control, event.top ,get_uvalue = info
  w_ysz=(*info).tlb_ysz-(*info).d_ysz
...
pro iris_xdetector_lineplot, event
  widget_control, event.top, get_uvalue = info
  thisevent=tag_names(event,/structure_name)
  case thisevent of
...
pro iris_xdetector_realsize, event
  widget_control, event.top, get_uvalue = info
...
; close iris_xdetector
pro iris_xdetector_destroy, event
  widget_control, event.top,/destroy
end

pro iris_xdetector_cleanup, tlb
...
pro iris_xdetector, data, lindx, group_leader = group_leader, $
               ncolors = ncolors, filename = filename
  if n_params() lt 2 then begin
    message, $
      'iris_xdetector: A data object and line index array must be given', $
       /cont
    return
  endif
...
; Set up the widgets
; base widget:
  xwt = 'Iris_Xdetector - ' + data->getregion(lindx[0],/full) + ' '+data->getfilename()  ; iris_xdetector window title
...
; create pulldown menus on the base widget menubar
  ...
  psmenu=widget_button(savemenu, value='Postscript', $
                       event_pro = 'iris_xdetector_ps')
  jpgmenu=widget_button(savemenu, value='JPG', event_pro = 'iris_xdetector_jpeg')
  exitmenu=widget_button(filemenu, value='Close', $
                         event_pro='iris_xdetector_destroy')

  optmenu=widget_button(menubar,value='Options', uvalue='options')
  colmenu=widget_button(optmenu, value='Colour table', $
                                 event_pro='iris_xdetector_colors')
  animenu=widget_button(optmenu, value='Create Animation', $
                                 event_pro='iris_xdetector_control_anim')
  wscalemenu=widget_button(optmenu, value='Change wavelength scale',/menu)
  angstr = string("305B)+'ngstr'+string("370B)+'m'
  pixmenu=widget_button(wscalemenu, value='Pixels',event_pro='iris_xdetector_wpix')
  angstrmenu=widget_button(wscalemenu, value=angstr,event_pro='iris_xdetector_wangstr')
...
;display window:
  ...
  drawid=widget_draw(displaybase, retain = 2,$
                     ...
                     /button_events, event_pro='iris_xdetector_zoom')
...
; create menu for controlling action in draw window
  ...
  dwoption_menu = cw_bgroup(dwoption, dwoption_names, /return_index, $
                            ...
                            event_func = 'iris_xdetector_dwoption')
  ...
  if nexp gt 1 then begin
    if nexpprp le 1 then begin 
      ...
      expslider = widget_slider(sliderbase, xsize=90, $
                              ...
                              title='Exp # ', $
                              value=0, $
                              event_pro='iris_xdetector_expslider',/drag)
    endif else begin
      exprp = 1
      expprpslider = widget_slider(sliderbase, xsize = 120, $
                                ...
                                value = 1, $
                                event_pro = 'iris_xdetector_expprp_slider')
      ...
      rastposslider = widget_slider(sliderbase, xsize = 120, $
                                ...
                                value = 1, $
                                event_pro = 'iris_xdetector_rast_slider')
    endelse
  endif
  ...
  pixelplot = widget_droplist(pixplotfield, value = pixnames, $
                              ...
                              event_pro = 'iris_xdetector_lineplot')
...
;  drawsizeoption_title = widget_label(drawsizeoption, value = 'Resize widget')
  ...
  drawsizeoption_menu = cw_bgroup(drawsizeoption, drawsizeoption_names, $
                            /return_index, $
                            /exclusive, set_value = 0, $
                            event_func = 'iris_xdetector_drawsizeoption')
  ...
  ; create pseudoevent and send this event to iris_xdetector_draw,
...
;  iris_xdetector_draw, pseudoevent
  ...
  xmanager, 'ql', tlb, /no_block, event_handler = 'iris_xdetector_resize', $
            group_leader = group, cleanup = 'iris_xdetector_cleanup'
