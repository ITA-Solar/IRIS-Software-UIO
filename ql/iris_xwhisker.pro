...
; PURPOSE:
;
;       iris_xwhisker is used to display 2-D spectroscopic data as whisker plots
...
;
; CALLING SEQUENCE:
;       iris_xwhisker, data, hdr, auxo, line, group_leader = group_leader
;
; INPUTS:
...
;
; PROCEDURE:
;       iris_xwhisker defines the widgets and displays data. It has several
;       options for displaying data in different modes, zooming,
;       selecting colors, file output etc. It is a QL-tool for
;       displaying data of 2 dimensions. iris_xwhisker consists
...
;
; $Id: iris_xwhisker.pro,v 1.35 2013/11/12 04:53:08 viggoh Exp $
;-
;
; save as postscript file
pro iris_xwhisker_ps,event
  thisfile=dialog_pickfile(/write,file='iris_xwhisker.ps')
  ...
  iris_xwhisker_draw,pseudoevent
  device,/close_file
  set_plot,thisdevice
end

; save as jpeg file
pro iris_xwhisker_jpeg,event
  thisfile=dialog_pickfile(/write,file='iris_xwhisker.jpg')
  if thisfile eq '' then return
  widget_control,event.top,get_uvalue=info
  wset,(*info).wid
  snapshot=tvrd()
...
;  pos[0]=pos[0]-(*(*info).data->getccd(region))[0]
  ...
  sit_and_stare=*(*info).data->getsit_and_stare((*info).line)
  if (*(*info).data->getaux())->getsscale() eq 'pixels' or $
     (*(*info).data->getaux())->getsscale() eq 'step nr' then begin
    ypos=indgen(sz[2]) 
  endif else begin
...
function iris_xwhisker_gamma, event
  widget_control, event.top,get_uvalue=info
...
function iris_xwhisker_histoopt, event
  widget_control, event.top,get_uvalue=info
...
function iris_xwhisker_dwoption, event
  widget_control, event.top, get_uvalue = info
...
pro iris_xwhisker_expprp_slider, event
  widget_control, event.top,get_uvalue=info
...
pro iris_xwhisker_slitslider, event
  widget_control, event.top,get_uvalue=info
...
;;   repeat begin
;;     ct=ct+1
;;     assoc_file = IRISxfiles_appReadme()+'/iris_xwhisker_ximovie_'+strtrim(string(ct),2)+'.tmp'
...
pro iris_xwhisker_sarcsec, event
  widget_control, event.top, get_uvalue = info
; change titles in aux object
...
pro iris_xwhisker_wangstr, event
  widget_control, event.top, get_uvalue = info
; change titles in aux object
...
; select color table
pro iris_xwhisker_colors, event
  widget_control, event.top, get_uvalue=info
  thisevent = tag_names(event, /structure_name)
  case thisevent of
  'WIDGET_BUTTON': begin
      xcolors, ncolors = (*info).ncolors, bottom = (*info).bottom, $
        title = 'iris_xwhisker colors (' + strtrim((*info).wid, 2) + ')', $
...
; protect colors
pro iris_xwhisker_protect_colors,event
  widget_control, event.top, get_uvalue = info
  tvlct, (*info).r, (*info).g, (*info).b, (*info).bottom
end

; resize main window
pro iris_xwhisker_resize, event
  widget_control, event.top ,get_uvalue = info
...
pro iris_xwhisker_lineplot, event
  widget_control, event.top, get_uvalue = info
  thisevent=tag_names(event,/structure_name)
  case thisevent of
...
; close iris_xwhisker
pro iris_xwhisker_destroy, event
  widget_control, event.top,/destroy
end

pro iris_xwhisker_cleanup, tlb
...
pro iris_xwhisker , data, line, group_leader = group_leader, $
              ncolors = ncolors
  if n_params() lt 2 then begin
    message, $
      'iris_xwhisker,data,line, group_leader = group,ncolors = ncolors',/cont
    return
  endif
...
;
  ...
  pixid = !d.window
; base widget:
  xwt = 'IRIS_Xwhisker - '+data->getfilename()   ; iris_xwhisker window title
...
; create pulldown menus on the base widget menubar
  ...
  psmenu=widget_button(savemenu, value='Postscript', event_pro = 'iris_xwhisker_ps')
  jpgmenu=widget_button(savemenu, value='JPG', event_pro = 'iris_xwhisker_jpeg')
  exitmenu=widget_button(filemenu, value='Close', event_pro='iris_xwhisker_destroy')

  optmenu=widget_button(menubar,value='Options', uvalue='options')
  colmenu=widget_button(optmenu, value='Colour table', $
                               event_pro='iris_xwhisker_colors')
  animenu=widget_button(optmenu, value='Create Animation', $
                               event_pro='iris_xwhisker_anim')
  wscalemenu=widget_button(optmenu, value='Change wavelength scale',/menu)
  angstr = string("305B)+'ngstr'+string("370B)+'m'
  pixmenu=widget_button(wscalemenu, value='Pixels',event_pro='iris_xwhisker_wpix')
  angstrmenu=widget_button(wscalemenu, value=angstr,event_pro='iris_xwhisker_wangstr')
  sscalemenu=widget_button(optmenu, value='Change spatial scale',/menu)
  pixmenu=widget_button(sscalemenu, value='Pixels',event_pro='iris_xwhisker_spix')
  angstrmenu=widget_button(sscalemenu, value='arcsec',event_pro='iris_xwhisker_sarcsec')
; display window:
  displaybase = widget_base(rcol, /row)
  drawid=widget_draw(displaybase, retain = 2,$
                     xsize = d_xsz, ysize = d_ysz, $
                       /button_events, event_pro='iris_xwhisker_zoom')
;
...
; create menu for controlling action in draw window
  ...
  dwoption_menu = cw_bgroup(dwoption, dwoption_names, /return_index, $
                            ...
                            event_func = 'iris_xwhisker_dwoption')
  ...
  if nexpprp gt 1 then begin
    ...
    expprpslider = widget_slider(sliderbase, xsize = 90, $
                                 ...
                                 value = 1, $
                                 event_pro = 'iris_xwhisker_expprp_slider')
  endif
  ...
  slitslider = widget_slider(sliderbase, xsize=90, $
                          ...
                          value=nslit/2, event_pro='iris_xwhisker_slitslider',/drag)
; control of gamma and histo_
   ...
   gamma_slider = cw_fslider(gammacol,/edit,format='(f6.2)',/frame, $
      ...
      event_func='iris_xwhisker_gamma',/drag)
   ...
   histoopt_slider = cw_fslider(gammacol,/edit,format='(f6.2)',/frame, $
      ...
      event_func='iris_xwhisker_histoopt',/drag)
...
; create pseudoevent and send this event to iris_xwhisker_draw,
; in order to draw the image
  pseudoevent={widget_button,id:0L, $
               top:tlb, handler:0l, select:1}
  iris_xwhisker_draw, pseudoevent

  xmanager, 'iris_xwhisker', tlb, /no_block, event_handler = 'iris_xwhisker_resize', $
            group_leader = group, cleanup = 'iris_xwhisker_cleanup'
