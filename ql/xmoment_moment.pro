...
;  keywords=pswindow()
  ...
  xmoment_moment_draw,pseudoevent
  device,/close_file
  set_plot,thisdevice
end

; save as jpeg file
pro xmoment_moment_jpeg,event
...
function xmoment_moment_wlen, event
  widget_control, event.top,get_uvalue=info
...
pro xmoment_moment_startslider, event
  widget_control, event.top,get_uvalue=info
...
pro xmoment_moment_stopslider, event
  widget_control, event.top,get_uvalue=info
...
pro xmoment_moment_line_startslider, event
  widget_control, event.top,get_uvalue=info
...
pro xmoment_moment_line_stopslider, event
  widget_control, event.top,get_uvalue=info
...
pro xmoment_moment_cont_startslider, event
  widget_control, event.top,get_uvalue=info
...
pro xmoment_moment_cont_stopslider, event
  widget_control, event.top,get_uvalue=info
...
pro xmoment_moment_setpix, event
  widget_control, event.top, get_uvalue = info
...
function xmoment_moment_pix_select, event
  widget_control, event.top, get_uvalue = info
  case event.value of
    0: begin
        (*info).setlpix = 1
...
;resize main window
pro xmoment_moment_resize, event
  widget_control, event.top ,get_uvalue = info
...
pro xmoment_moment_destroy, event
       widget_control, event.top, /destroy
end

pro xmoment_moment_cleanup, tlb
...
pro xmoment_moment, data, mspec, iwin, lambda, wlref=wlref, groupl = groupl, $
             xtitle = xtitle, ytitle = ytitle

  if n_params() lt 4 then begin
    print, 'xmoment_moment, data, mspec, iwin, lambda, wlref=wlref, wgroupl = groupl, $'
...
;  stop = lambda[(size(lambda))[1]-1]
  if nlam ne nspec then begin
    print, 'xmoment_moment: Wavelength and intensity arrays must be of same length'
           return
  endif
...
;  cpix = [0,0]
  ...
  drawid=widget_draw(displaybase, retain = 2, xsize = d_xsz, ysize = d_ysz, $
                     /button_events, event_pro = 'xmoment_moment_setpix')
  filemenu=widget_button(menubar, value='File',/menu, uvalue='file')
  savemenu=widget_button(filemenu, value='Save as', /menu)
  psmenu=widget_button(savemenu, value='Postscript', event_pro = 'xmoment_moment_ps')
  jpgmenu=widget_button(savemenu, value='JPG', event_pro = 'xmoment_moment_jpeg')

  exitmenu=widget_button(filemenu, value='Close', event_pro='xmoment_moment_destroy')
;
...
;                         pix_id, event_func = 'xmoment_moment_pix_select')
  ...
  startslider = widget_slider(lcol, xsize=120, $
                          ...
                          title=starttitle, $
                          value=start, event_pro = 'xmoment_moment_startslider', $
                          /drag)
  stopslider = widget_slider(lcol, xsize=120, $
                          minimum=start, maximum=stop, $
                          title=stoptitle, $
                          value=stop, $
                          event_pro = 'xmoment_moment_stopslider', /drag)

  linelabel = widget_label(lcol, value='Define Line')
;
;  if moment type of data then give option of changing ref wavelength
;
  wlen_ref=data->getline_wvl(iwin,wscale='AA')
  if keyword_set(wlref) then begin
    line_wlenslider = cw_fslider(lcol,/edit,format='(f7.2)',/frame, $
      ...
      value=wlen_ref,event_func='xmoment_moment_wlen',/drag)
  endif
  ...
  line_startslider = widget_slider(lcol, xsize=120, $
                          ...
                          value=lpix[0], event_pro = 'xmoment_moment_line_startslider', $
                          /drag)
  line_stopslider = widget_slider(lcol, xsize=120, $
                          minimum=start, maximum=stop, $
                          title=line_stoptitle, $
                          value=lpix[1], $
                          event_pro = 'xmoment_moment_line_stopslider', /drag)
...
; widget_control, line_stopslider, set_value=stop ; 2-Jan-2008
  ...
  xmoment_moment_resize, pseudoevent
;  pseudoevent={widget_button,id:0L, $
;    top:tlb, handler:0l, select:1}
;  xmoment_moment_draw, pseudoevent
  if groupl ne -1 then begin
  xmanager, 'xmoment_moment', tlb, group_leader = groupl, $
             event_handler = 'xmoment_moment_resize', cleanup = 'xmoment_moment_cleanup'
  endif else begin
  xmanager, 'xmoment_moment', tlb, $
             event_handler = 'xmoment_moment_resize', cleanup = 'xmoment_moment_cleanup'
  endelse
end
