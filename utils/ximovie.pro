...
pro ximovie, afile, xsize, ysize, afile2 = afile2, offset = offset, pos = pos, $
  ...
  if n_params() lt 1 then begin
    ...
    print,  'nframes = nframes, first_im=first_im, title=title, $'
    ...
    print, 'swap_endian=swap_endian, time=time,clock=clock'
    return
  endif
  ...
  case type of
   'double' : begin
    ...
    if not keyword_set(nframes) then nframes = status.size/xsize/ysize/8
